...
public class HexagonGameP2P extends JFrame {
    ...
    static class Cell {
        ...
        Shape hexPath;   // A smoothed version of the polygon
        int centerX, centerY;
        ...
        private Polygon createHexagon(int centerX, int centerY, int size) {
            ...
            // For a pointy-topped hexagon, vertices are at 30°, 90°, …, 330°.
            for (int i = 0; i < 6; i++) {
    ...
    class PeerNetwork implements Runnable {
        ...
        public PeerNetwork() {
            try {
                ...
                // Use the new joinGroup method:
                NetworkInterface ni = NetworkInterface.getByInetAddress(InetAddress.getLocalHost());
                socket.joinGroup(new InetSocketAddress(group, PORT), ni);
                // Leave loopback enabled so that local instances receive multicast packets.
                // We'll filter self-messages using the instanceId.
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
