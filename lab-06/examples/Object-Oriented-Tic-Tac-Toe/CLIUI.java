...
/**
 * CLI interface for Tic Tac Toe.
 * Handles input/output in console.
 */
public class CLIUI implements TicTacToeUI {
    ...
    public void start(TicTacToeGame game, boolean vsAI, Opponent ai, TicTacToeGame.Mark aiPlaysAs) {
        ...
        while (!game.isGameOver()) {
            if (vsAI && game.getCurrent() == aiAs) {
                ...
                int move = promptMove();
                if (!game.play(move)) System.out.println("Illegal move. Try again.");
            }
        }
    }
    ...
    @Override
    public void onMove(int index, TicTacToeGame.Mark who) { draw(game.getBoard()); }
